...
async function openPDF(bytes,name,{handle=null}={}) {
  ...
  let task, next;
  try {
    const digest=await fingerprint(bytes),key='ravibandara-paper-review:v1:'+digest;
...
async function downloadMarkedPDF() {
  ...
  try {
    ...
    if(result.mode==='file') {
      ...
      const previousKey=storageKey;storageKey='ravibandara-paper-review:v1:'+result.digest;activeStorageKeys.add(storageKey);persist();
...
for(const link of document.querySelectorAll('.back-link,.brand-icon'))link.addEventListener('click',event=>{
  ...
  if(pdf&&(!confirm(leaveMessage('Open GitHub repository?'))||!clearWorkspace())){event.preventDefault();return;}
...
// Optional agent navigation uses the same visible viewer actions.
if (document.modelContext?.registerTool) {
 const lifecycle = new AbortController();
 try { Promise.resolve(document.modelContext.registerTool({
  name: 'navigate_review_page', title: 'Navigate PDF review',
  description: 'Navigate the main or companion view of the open PDF to a page.',
  inputSchema: {type:'object',properties:{side:{type:'string',enum:['left','right']},page:{type:'integer',minimum:1}},required:['side','page'],additionalProperties:false},
  annotations: {readOnlyHint:false,untrustedContentHint:false},
  execute(input) {
   if (!pdf) throw new Error('Open a PDF first.');
   if (!input || !['left','right'].includes(input.side) || !Number.isInteger(input.page) || input.page<1 || input.page>pdf.numPages) throw new Error('Choose a valid view and PDF page.');
   sides[input.side].goToPage(input.page);
   return {side:input.side,page:input.page};
  }
