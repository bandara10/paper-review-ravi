...
export function upgradeWorkspaceUI() {
  ...
  document.querySelector('.version').textContent = 'ALPHA 3';
  document.querySelector('.help-content').innerHTML = `
    ...
    <h3>Annotations already in your PDF</h3><p>Existing highlights and other PDF marks are visible in both views. Choose Select and click a small yellow comment icon to read an embedded note or comment thread. Comments created in other PDF apps are read-only here and are retained when you save. They are separate from your review notebook. Comments stored only in another app must first be saved into the PDF to appear here.</p>
    ...
    <h3>Before leaving a review</h3><p>Opening another PDF, closing the PDF, or opening the GitHub repository asks for confirmation before clearing the current workspace's comments, annotations, and extracted references. Save PDF and export your comments first. Cancelling keeps your review open. A PDF that fails to open does not clear the current review.</p><p>Closing or reloading the tab uses the browser's standard leave warning. Browsers choose its wording and may suppress it, especially on mobile. Clearing runs when a normal page exit is reported. Switching tabs does not clear the review.</p>
